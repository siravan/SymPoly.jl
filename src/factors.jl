...
sym(f::FactoredPoly) = prod(map(v -> sym(first(v))^last(v), factors(f)); init=1)
poly(f::FactoredPoly) = prod(map(v -> first(v)^last(v), factors(f)); init=1)
...
function Primes.factor(p::Poly; verbose=false)
    ...
    for w in factors(f)
        v, k = first(w), last(w)
        if degree(v) > 0
            for w in factors(factor_schubert_kronecker(v))
